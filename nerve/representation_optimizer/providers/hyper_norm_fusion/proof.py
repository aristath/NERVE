...
class ExactHyperNormFusionProofVerifier:
        ...
        try:
            if request.obligation not in EXACT_FUSION_OBLIGATIONS:
                raise ModelCompileError(
                    f"unsupported hyper/RMS proof obligation {request.obligation!r}"
                )
            ...
            if (
                ...
                raise ModelCompileError(
                    "hyper/RMS proof contracts belong to another candidate"
                )
    ...
    def iter_proof_artifact(
        ...
        *,
        ...
        if (
            ...
            raise ModelCompileError(
                "hyper/RMS proof artifact chunk size must be positive"
            )
        ...
        if (
            ...
            raise ModelCompileError(
                "hyper/RMS proof artifact reference is invalid"
            )
        with _regular_file(
    ...
    def _candidate_root(self, candidate_id: str) -> Path:
        if (
            ...
            raise ModelCompileError("hyper/RMS proof candidate identity is unsafe")
        ...
        if not root.is_relative_to(workspace) or root.is_symlink() or not root.is_dir():
            raise ModelCompileError(
                "hyper/RMS proof candidate bundle is unavailable"
            )
        return root
...
def _verify_candidate(
    *,
    ...
    for record in lowering["shader_artifacts"]:
        ...
        if actual != expected:
            raise ModelCompileError(
                f"hyper/RMS shader artifact {path!r} is not deterministic"
            )
    ...
    for opportunity in opportunities:
        ...
        if overlay != expected_overlay:
            raise ModelCompileError(
                f"hyper/RMS overlay for {opportunity.component_id!r} changed outside its proof"
            )
    ...
    if proof != expected_proof:
        raise ModelCompileError("hyper/RMS proof certificate is inconsistent")
    return verified
...
def _regular_file(root: Path, relative_path: str) -> Path:
    ...
    if (
        ...
        raise ModelCompileError(
            f"hyper/RMS proof artifact path is unsafe: {relative_path!r}"
        )
    path = root
    for part in relative.parts:
        ...
        if path.is_symlink():
            raise ModelCompileError(
                f"hyper/RMS proof path crosses a symlink: {relative_path!r}"
            )
    root = root.resolve()
    resolved = path.resolve()
    if not resolved.is_relative_to(root) or not resolved.is_file():
        raise ModelCompileError(
            f"hyper/RMS proof artifact is unavailable: {relative_path!r}"
        )
    return resolved
