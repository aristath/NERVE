...
def hyper_norm_representation_graph(
    *,
    candidate: Json,
    ...
    if not opportunities or len(opportunities) != len(prepared):
        raise ModelCompileError(
            "hyper/RMS representation requires matching component alternatives"
        )
    ...
    for index, (opportunity, component) in enumerate(
        ...
        source_node_ids = sorted(
            ...
            for node_id in region.semantic_source_node_ids
        )
        ...
        node_id = f"node.hyper_norm_island.{prefix}"
        ...
        nodes.append(
            {
                "id": node_id,
                "kind": "operator",
                "operation": "exact_hyper_connection_rms_norm_island",
                "inputs": [
                    {
                        "id": "input",
                ...
                "cost": {
                    "status": "estimated",
                    "metrics": {
                        "source_dispatches": float(3 * len(opportunity.regions)),
                        "candidate_dispatches": float(len(opportunity.regions)),
                    },
                },
                "provenance": provenance(),
            }
        ...
        physical_kernels.extend(
            {
                ...
                "cost": {
                    "status": "estimated",
                    "metrics": {"dispatch_reduction": 2.0},
                },
                "provenance": provenance(),
            }
    ...
    return finalize_representation_graph(
        {
            ...
            "physical_representations": [
                ...
                },
                {
                    ...
                    "domain": "topology",
                    "physical_shape": [len(opportunities[0].regions)],
