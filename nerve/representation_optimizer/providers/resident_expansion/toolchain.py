...
class ResidentExpansionSemanticConstructor:
    def construct_semantic_artifacts(
        ...
        for record in lowering["resident_derivations"]:
            ...
            derived = mxfp4_to_fp8_resident_derivation(
                ...
                {
                    "devices": [
                        {"shader_features": record["derivation"]["required_features"]}
                    ]
                },
            )
            if (
        ...
        context.write_json_artifact(
            PROOF_PATH,
            {
                ...
                "derivation_count": len(lowering["resident_derivations"]),
                "derivations_digest": contract_digest(
                    {"resident_derivations": lowering["resident_derivations"]}
                ),
        ...
        context.write_json_artifact(
            ...
            component_fixture(
                component_id=lowering["source"]["component_id"],
                node_ids=tuple(lowering["source"]["node_ids"]),
...
class ResidentExpansionOrdinaryRelowerer:
    def run_ordinary_lowering(
        ...
        lowering = _lowering(context)
        manifest_ref = lowering["source"]["manifest_ref"]
        manifest = _json_object(
            context.read_source_artifact(manifest_ref),
            manifest_ref,
        )
        component_id = lowering["source"]["component_id"]
        component = deepcopy(
        ...
        implementation = "optimized_exact_resident_parameter_expansion_v1"
        ...
        if len(replacement_counts) != len(lowering["shader_replacements"]):
            raise ModelCompileError(
