...
def attention_benchmark_workloads(
    ...
    common = {
        ...
        "context_size": opportunity.local_window
        + opportunity.max_compressed_indices,
        "state_size": opportunity.local_window
        + opportunity.max_compressed_indices,
        "stream_count": 1,
        "mount_mode": "resident_reuse",
        "boundary_mode": "local",
        "input_artifact": fixture,
        "initial_state_artifact": fixture,
        ...
        "useful_work_unit": "complete_attention_transactions",
    ...
    return tuple(
        create_benchmark_workload(
            name=(
                f"grouped-head attention hg{opportunity.head_group} {phase}"
            ),
...
def attention_validation_requirements(
    *,
    candidate: Json,
    opportunity: AttentionHeadGroupingOpportunity,
    ...
    checks = [
        create_validation_check(
            name=f"grouped-head attention {phase} equivalence",
            stage="sanity",
            ...
            initial_state_artifact=component_ref,
            controls={
                "phase": phase,
                "component_id": opportunity.component_id,
                "physical_node_id": opportunity.physical_node_id,
            ...
            seeds=(1,),
            step_unit="attention_transactions",
    ...
    checks.extend(
        (
            _whole_model_check(
                name="grouped-head attention fixed-token replay",
                stage="full_local",
                kind="teacher_forced",
            ...
            _whole_model_check(
                name="grouped-head attention lifecycle continuity",
                stage="full_local",
            ...
            _whole_model_check(
                name="grouped-head attention graph duplication",
                stage="full_local",
                kind="graph_edit",
            ...
            _whole_model_check(
                name="grouped-head attention alternative placement",
                stage="full_local",
                kind="placement",
            ...
            create_validation_check(
                name="grouped-head attention reasoning and long stream",
                stage="whole_model",
            ...
            create_validation_check(
                name="grouped-head attention product conversation performance",
                stage="whole_model",
                kind="free_running",
    ...
    return create_validation_requirements(
        ...
        not_applicable_reasons={
            coverage: (
                "the exact attention schedule does not own this semantic responsibility"
            )
...
def _component_fixture(opportunity: AttentionHeadGroupingOpportunity) -> Json:
