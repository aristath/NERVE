...
from nerve.representation_optimizer.providers.parallel_projection_fusion.discovery import (
    ...
    discovery_result,
    is_parallel_projection_scope,
    source_inputs,
)
from nerve.representation_optimizer.providers.parallel_projection_fusion.physical import (
    prepare_fused_component,
)
from nerve.representation_optimizer.providers.parallel_projection_fusion.representation import (
    parallel_projection_representation_graph,
)
from nerve.representation_optimizer.providers.parallel_projection_fusion.workloads import (
...
class ExactParallelProjectionFusionProvider:
    identity = ProviderIdentity("nerve.exact_parallel_projection_fusion", "1")
    ...
    def emit_representation_ir(
        ...
        return parallel_projection_representation_graph(
            candidate=candidate,
    ...
    def estimate_static_cost(
        ...
        return StaticEstimate(
            ...
            steady_state_work={
                "kind": "exact_parallel_projection_island",
                ...
                "removed_dispatch_count": (
                    source_dispatch_count - candidate_dispatch_count
                ),
            },
            reasons=(
                "the candidate keeps source parameters, prequantization, and state unchanged while combining independent projection dispatches",
            ),
        )
    ...
    def benchmark_workloads(
        ...
        return parallel_projection_benchmark_workloads(
    ...
    def validation_requirements(
        ...
        return parallel_projection_validation_requirements(
            candidate=candidate,
...
def _opportunity_groups(
    context: ProviderContext,
) -> tuple[tuple[ParallelProjectionFusionOpportunity, ...], ...]:
    grouped: dict[str, list[ParallelProjectionFusionOpportunity]] = {}
    ...
    return tuple(
        tuple(
            sorted(
                group,
                key=lambda item: (item.component_id, item.physical_node_id),
            )
        )
...
def _candidate_opportunities(
    ...
    if not matches or covered != requested:
        raise ModelCompileError(
            "parallel projection candidate does not map to exact component regions"
        )
    matches = tuple(
        sorted(matches, key=lambda item: (item.component_id, item.physical_node_id))
    )
    expected = sorted(
        (
            (scope_id, digest)
    ...
    if list(
        ...
        raise ModelCompileError("parallel projection candidate source contracts drifted")
    ...
    if (
        ...
        raise ModelCompileError(
            "parallel projection candidate crosses physical performance classes"
        )
    return matches
...
def _lowered_component(opportunity: ParallelProjectionFusionOpportunity) -> Json:
    return {
        "component_id": opportunity.component_id,
        ...
        "region": {
            "scope_ids": list(opportunity.region.scope_ids),
            "source_contract_digests": list(
                opportunity.region.source_contract_digests
            ),
...
def representative_opportunity(
    ...
    if not opportunities:
        raise ModelCompileError("parallel projection lowering has no opportunities")
