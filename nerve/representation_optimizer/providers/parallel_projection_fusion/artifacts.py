...
PROOF_PATH = "proofs/exact_parallel_projection_fusion.json"
COMPONENT_FIXTURE_PATH = "fixtures/parallel_projection_component.json"
...
def kernel_artifact_path(shader_file: str) -> str:
    ...
    if (
        ...
        raise ModelCompileError(
            f"unsupported fused parallel projection shader path {shader_file!r}"
        )
    return f"kernels/parallel_projection/{path.with_suffix('.spv').name}"


def component_overlay_path(component_id: str, physical_node_id: str) -> str:
    return (
        "overlays/"
        f"{stable_contract_id('parallel_projection_region', component_id, physical_node_id)}.json"
    )
...
def component_fixture(
    *,
    ...
    return {
        "schema": "nerve.optimizer.parallel_projection_component_fixture.v1",
