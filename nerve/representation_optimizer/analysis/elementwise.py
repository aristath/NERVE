...
class ElementwiseStructureAnalyzer:
    ...
    def analyze(self, context: ScopeAnalysisContext) -> AnalyzerResult:
        ...
        for parameter in context.parameters:
            ...
            zero_status = (
                "supported"
                if all_zero and observation.exhaustive
                else "inconclusive"
                if all_zero
                else "rejected"
            )
            claims.append(
                claim(
                    ...
                    and (zero_status == "rejected" or exactly_zero),
                    facts={
            ...
            constant_status = (
                "supported"
                if constant and observation.exhaustive
                else "inconclusive"
                if constant
                else "rejected"
            )
            ...
            codebook_supported = (
                codebook_ratio <= context.budget.codebook_ratio_threshold
            )
