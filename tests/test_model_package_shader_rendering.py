...
def test_compiler_vectorizes_small_fp8_batches_across_shared_weights(
    ...
    for shader_file in grouped_shader_files:
        source = (tmp_path / shader_file).read_text()
        assert_uses_direct_packed_fp8_vectors(source)
        assert "const uint BATCH_GROUP_WIDTH = 4u;" in source
        assert "float sums[BATCH_GROUP_WIDTH];" in source
        assert "shared float output_rows[BATCH_GROUP_WIDTH]" in source
    ...
    for shader_file in row_scaled_shader_files:
        ...
        assert_uses_direct_packed_fp8_vectors(source)
        assert "const uint BATCH_TILE_WIDTH = 4u;" in source
        assert "const uint BATCH_GROUP_WIDTH" not in source
...
def test_compiler_renders_sigmoid_gate_with_reusable_fp8_output(
    tmp_path: Path,
) -> None:
    shader_source_dir = Path(__file__).parents[1] / "runtime-rs" / "shaders"
    shader_files = {
        "sigmoid_multiply_quantize_fp8_e4m3_b128_h6144.comp",
        "sigmoid_multiply_quantize_batch4_fp8_e4m3_b128_h6144.comp",
    }

    copy_shader_templates(shader_source_dir, tmp_path, shader_files)

    decode = (
        tmp_path / "sigmoid_multiply_quantize_fp8_e4m3_b128_h6144.comp"
    ).read_text()
    batch = (
        tmp_path
        / "sigmoid_multiply_quantize_batch4_fp8_e4m3_b128_h6144.comp"
    ...
    for source in (decode, batch):
        assert "layout(local_size_x = 1024" in source
        assert "const uint ELEMENT_COUNT = 6144u;" in source
        assert "const uint BLOCK_COLUMNS = 128u;" in source
        assert "subgroupMax" in source
        assert "{{" not in source
    assert "layout(push_constant) uniform BatchControl" not in decode
