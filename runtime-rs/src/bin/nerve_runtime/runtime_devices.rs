...
const RUNTIME_PHYSICAL_DEVICE_MEMORY_RESTORATION_SCHEMA: &str =
    "nerve.runtime.physical_device_memory_restoration.v2";

#[derive(Clone, Debug, PartialEq, Eq, serde::Serialize)]
struct RuntimePhysicalDeviceMemoryObservation {
...
#[derive(Clone, Debug, PartialEq, Eq, serde::Serialize)]
struct RuntimePhysicalDeviceMemoryRestorationReport {
    schema: &'static str,
    complete: bool,
    physical_device_count: usize,
    restored_device_count: usize,
    devices: Vec<RuntimePhysicalDeviceMemoryRestorationDeviceReport>,
    errors: Vec<String>,
}

fn capture_runtime_physical_device_memory_observations(
    args: &Args,
    physical_device_ids: &BTreeSet<String>,
) -> Result<Vec<RuntimePhysicalDeviceMemoryObservation>, nerve_runtime::VulkanError> {
    if physical_device_ids.is_empty() {
        return Err(nerve_runtime::VulkanError(
    ...
    physical_device_ids
        ...
        .map(|physical_device_id| {
            let info = info_by_id.get(physical_device_id.as_str()).ok_or_else(|| {
                nerve_runtime::VulkanError(format!(
            ...
            Ok(RuntimePhysicalDeviceMemoryObservation {
...
fn verify_runtime_physical_device_memory_restoration(
    before: &[RuntimePhysicalDeviceMemoryObservation],
    after: &[RuntimePhysicalDeviceMemoryObservation],
) -> RuntimePhysicalDeviceMemoryRestorationReport {
    ...
    RuntimePhysicalDeviceMemoryRestorationReport {
        schema: RUNTIME_PHYSICAL_DEVICE_MEMORY_RESTORATION_SCHEMA,
