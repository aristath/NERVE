...
fn runtime_hybrid_representation_placement_candidates(
    ...
    plans
        .into_iter()
        .map(|plan| {
            runtime_hybrid_representation_placement_from_plan(
                &candidates,
                exact_baseline_incompatible_instance_ids,
                plan,
            )
...
fn visit_runtime_hybrid_representation_placements_by_duration<T, F>(
    runtime_model: &VulkanResidentRuntimeModel,
    applications: &[VulkanRuntimeHybridRepresentationApplication],
    exact_baseline_incompatible_instance_ids: &BTreeSet<String>,
    catalog: &VulkanPlacementCalibrationCatalog,
...
where
    F: FnMut(
        VulkanRuntimeHybridRepresentationPlacement,
    ) -> Result<Option<T>, VulkanRuntimeHybridPlacementError>,
{
    let candidates = runtime_hybrid_representation_candidate_graph(
        runtime_model,
    ...
    visit_vulkan_hybrid_ordered_graph_routes_by_duration(
        ...
        |route| {
            let placement = runtime_hybrid_representation_placement_from_plan(
                &candidates,
                exact_baseline_incompatible_instance_ids,
                VulkanHybridPlacementPlan {
                    steps: route.steps.clone(),
                    predicted_duration_ns_per_activation: route
                        .predicted_duration_ns_per_activation,
                    resource_reservations: route.calibration_resource_reservations.clone(),
                },
            )
            .map_err(|error| VulkanHybridPlacementError(error.to_string()))?;
            if !runtime_hybrid_route_execution_transient_fits(
                exact_parameter_planner,
                runtime_model,
                phase,
                &placement.ordered_placement,
                route,
                eligible_capacity,
            )
            .map_err(|error| VulkanHybridPlacementError(error.to_string()))?
            ...
            visitor(placement).map_err(|error| VulkanHybridPlacementError(error.to_string()))
        },
    )
    .map_err(|error| VulkanRuntimeHybridPlacementError(error.to_string()))
}

fn runtime_hybrid_representation_placement_from_plan(
    candidates: &VulkanRuntimeHybridCandidateGraph,
    exact_baseline_incompatible_instance_ids: &BTreeSet<String>,
    plan: VulkanHybridPlacementPlan,
) -> Result<VulkanRuntimeHybridRepresentationPlacement, VulkanRuntimeHybridPlacementError> {
    ...
    let selected_region_executions = plan
        ...
        .collect();
    Ok(VulkanRuntimeHybridRepresentationPlacement {
...
) -> Result<Option<VulkanRuntimeHybridPhysicalExecutionResolution>, VulkanRuntimeHybridPlacementError>
{
    let prefill_activation_batch_width =
        vulkan_runtime_hybrid_calibrated_prefill_widths(runtime_model, catalog)?
            .into_iter()
...
) -> Result<Option<VulkanRuntimeHybridPhysicalExecutionResolution>, VulkanRuntimeHybridPlacementError>
{
    ...
    let applications = runtime_model
        ...
        .map_err(|error| VulkanRuntimeHybridPlacementError(error.to_string()))?;
    // Bounded component-calibration memory is ordering evidence, never final
    // admission or a route-discovery constraint. Decode routes are visited
    // lazily by predicted duration without sampled-resource Pareto pruning;
    // the full lowered package plan below is the terminal verifier.
                ...
                let resolution = visit_vulkan_runtime_hybrid_ordered_graph_with_owners_by_duration(
                    ...
                    |prefill| {
                        try_resolve_vulkan_runtime_hybrid_phase_set_mount(
                            package_root,
                            &mounted_model,
                            decode.ordered_placement.clone(),
                            Some(prefill),
                            catalog,
                            context_capacity_activations,
                            logical_device_id_by_physical_device,
                            physical_mount_planning.as_ref(),
                        )
                    },
                )?;
...
#[allow(clippy::too_many_arguments)]
fn try_resolve_vulkan_runtime_hybrid_phase_set_mount(
    package_root: &Path,
    mounted_model: &VulkanResidentRuntimeModel,
    decode: VulkanRuntimeHybridOrderedPlacement,
    prefill: Option<VulkanRuntimeHybridOrderedPlacement>,
    catalog: &VulkanPlacementCalibrationCatalog,
    context_capacity_activations: usize,
    logical_device_id_by_physical_device: &BTreeMap<String, String>,
    physical_mount_planning: Option<&VulkanRuntimeHybridMountPlanningContext<'_>>,
...
{
    let placement = VulkanRuntimeHybridPhaseSetPlacement { decode, prefill };
    let decode_predicted_duration_ns_per_activation =
        placement.decode.plan.predicted_duration_ns_per_activation;
    let prefill_predicted_duration_ns_per_activation = placement
        .prefill
    ...
    let physical_mount_plan = if let Some(planning) = physical_mount_planning {
...
fn try_plan_vulkan_runtime_hybrid_ordered_graph_with_owners_and_filter(
    ...
    .map_err(|error| VulkanRuntimeHybridPlacementError(error.to_string()))?;
    Ok(plan.map(|plan| runtime_hybrid_ordered_placement_from_plan(&candidates, plan)))
}

fn visit_vulkan_runtime_hybrid_ordered_graph_with_owners_by_duration<T, F>(
    runtime_model: &VulkanResidentRuntimeModel,
    catalog: &VulkanPlacementCalibrationCatalog,
    eligible_capacity: &VulkanPlacementCapacityEnvelope,
    phase: VulkanTargetedComponentExecutionPhase,
    ...
    ) -> Result<Option<T>, VulkanRuntimeHybridPlacementError>,
{
    ...
    visit_vulkan_hybrid_ordered_graph_routes_by_duration(
        ...
        eligible_capacity,
        |route| {
            let placement = runtime_hybrid_ordered_placement_from_plan(
                &candidates,
                VulkanHybridPlacementPlan {
                    steps: route.steps.clone(),
                    predicted_duration_ns_per_activation: route
...
fn runtime_hybrid_route_execution_transient_fits(
    planner: Option<&VulkanRuntimeHybridExactCandidateResourcePlanner<'_>>,
    runtime_model: &VulkanResidentRuntimeModel,
    phase: VulkanTargetedComponentExecutionPhase,
    placement: &VulkanRuntimeHybridOrderedPlacement,
    route: &VulkanHybridPlacementRoute,
    capacity: &VulkanPlacementCapacityEnvelope,
) -> Result<bool, VulkanRuntimeHybridPlacementError> {
    let Some(planner) = planner else {
        return Ok(true);
    };
    let claims = planner.route_execution_transient_claims(runtime_model, phase, placement)?;
    if claims.is_empty() {
        return Ok(true);
    }
    route
        .authoritative_resource_reservations
        .reserve(&VulkanHybridCandidateResources::new(claims), capacity)
        .map(|reservation| reservation.is_some())
        .map_err(|error| VulkanRuntimeHybridPlacementError(error.to_string()))
}

fn runtime_hybrid_ordered_placement_from_plan(
    candidates: &VulkanRuntimeHybridCandidateGraph,
    plan: VulkanHybridPlacementPlan,
) -> VulkanRuntimeHybridOrderedPlacement {
    let selected_region_executions = plan
...
fn runtime_hybrid_candidate_graph(
        ...
        component_ids.iter().zip(&component_targets).enumerate()
    {
        ...
        for (candidate_index, observation) in catalog
            ...
            .into_iter()
            .filter(|observation| match component_strategy_filter {
    ...
    for (region_index, calibration) in catalog.region_executions().iter().enumerate() {
        if component_strategy_filter == VulkanRuntimeHybridComponentStrategyFilter::SingleDeviceOnly
        {
            continue;
        }
        ...
        for component_start in 0..=component_ids.len() - region_len {
            ...
            region_candidates.push(VulkanHybridRegionCandidate {
                ...
                component_start,
                component_end,
                semantic_contract_id: outer.behavior.compiled_execution_signature.clone(),
...
fn runtime_hybrid_exact_replaced_resource_classes() -> BTreeSet<VulkanHybridResourceClass> {
    let mut classes = runtime_hybrid_exact_authoritative_resource_classes();
    classes.insert(VulkanHybridResourceClass::ExecutionTransient);
    classes
}
...
fn apply_runtime_hybrid_exact_candidate_resources(
    ...
    let classes = runtime_hybrid_exact_replaced_resource_classes();
...
fn apply_runtime_hybrid_exact_boundary_resources(
    resource_catalog: &mut VulkanHybridCandidateResourceCatalog,
    runtime_model: &VulkanResidentRuntimeModel,
    ...
    for candidate in boundary_candidates {
        ...
        let base_byte_count = candidate.byte_count.checked_div(width).filter(|base| {
            base.checked_mul(width) == Some(candidate.byte_count)
        }).ok_or_else(|| {
            VulkanRuntimeHybridPlacementError(
                "exact hybrid boundary byte geometry is not divisible by its lane width"
                    .to_string(),
            )
        })?;
        for class in &classes {
            let mut claims = Vec::new();
            if *class == VulkanHybridResourceClass::MutableState {
                ...
                claims.extend(requirements.into_iter().map(|requirement| {
                    match requirement.target {
                        ...
                        VulkanHybridResourceTarget::Host => VulkanHybridResourceClaim::host(
                            requirement.resource_identity,
                            requirement.class,
                            requirement.byte_count,
                        ),
            ...
            if *class == VulkanHybridResourceClass::ExecutionTransient
                && matches!(phase, VulkanTargetedComponentExecutionPhase::Prefill { .. })
                && candidate
                    .execution_case
                    .transports
                    .first()
                    .is_some_and(|transport| {
                        runtime_mounted_boundary_route(&transport.route)
                            == Ok(VulkanPlacedEdgeTransferRoute::DeviceLocalStaging)
                    })
            {
                claims.push(VulkanHybridResourceClaim::exclusive_host(
                    format!(
                        "runtime-boundary:{}:{}:{}:prefill-staging",
                        candidate.boundary_index,
            ...
            resource_catalog
                .replace_boundary_resource_class_claims(
                    candidate.boundary_index,
                    &candidate.execution_case,
                    *class,
                    claims,
...
fn runtime_hybrid_representation_candidate_graph(
    ...
    for (application_index, application) in applications.iter().enumerate() {
        ...
        for candidate in combined.region_candidates.iter_mut().filter(|candidate| {
            ...
            })
        {
            ...
            let exact_shared_requirements = alternative
                .exact_shared_requirements_by_candidate_id
            ...
            if let Some(requirements) = exact_shared_requirements {
                combined
                    .exact_shared_requirements_by_candidate_id
...
pub fn vulkan_runtime_hybrid_calibrated_prefill_widths(
    ...
    let candidate_widths = catalog
        .candidate_activation_batch_widths_for_runtime_phase(
            crate::RUNTIME_IMPLEMENTATION_FINGERPRINT,
            nerve_execution_contracts::ExecutionPhase::Prefill,
        )
        .into_iter()
        .filter(|activation_batch_width| *activation_batch_width >= 2)
        .collect::<BTreeSet<_>>();
...
fn runtime_hybrid_step_component_cases<'a>(
    ...
    let calibration = placement
        ...
        .ok_or_else(|| {
            VulkanRuntimeHybridPlacementError(
                "runtime hybrid multi-component region has no exact replay calibration".to_string(),
            )
        })?;
...
> {
    ...
    if let Some(prefill) = prefill {
        physical_execution_plan.prefill_activation_batch_width =
            Some(prefill.activation_batch_width);
...
pub fn lower_vulkan_runtime_hybrid_phase_placement(
    ...
    let mut scheduled_boundaries = Vec::<(usize, &VulkanPlacementExecutionCaseIdentity)>::new();
    for step in &placement.plan.steps {
        match step {
