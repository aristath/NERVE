...
fn distributed_calibration_transient_backing_bytes(
    ...
    let shared_allocations = plan
        ...
        .collect::<Vec<_>>();
    let shared_byte_capacity = shared_allocations.iter().try_fold(
        0usize,
        |total, (_, byte_capacity)| total.checked_add(*byte_capacity),
    )
    .ok_or_else(|| {
        distributed_calibration_error_value(
...
mod runtime_distributed_placement_calibration_strategy_tests {
    ...
    fn accounts_reduction_and_private_transient_backing() {
        let devices = vec!["gpu-a".to_string(), "gpu-b".to_string()];
        let plan = |route| VulkanDistributedActivationBufferPlan {
        ...
        let (shared_host_devices, shared_host_bytes) =
            distributed_calibration_transient_backing_bytes(
                &devices,
                &plan(VulkanSharedResidentBufferRoute::SharedHost),
            )
            .unwrap();
        ...
        let (device_local, host_bytes) = distributed_calibration_transient_backing_bytes(
            &devices,
            &plan(VulkanSharedResidentBufferRoute::ExternalDeviceLocal),
        )
        .unwrap();
    ...
    fn rejects_incomplete_transient_backing_totals() {
        ...
        let mut plan = VulkanDistributedActivationBufferPlan {
            allocations: vec![VulkanDistributedActivationBufferAllocation {
                storage: VulkanDistributedActivationStorage::ActivationSlot,
                owner_device_id: "gpu-a".to_string(),
                component_id: "component".to_string(),
            ...
            import_count: 1,
            reference_count: 1,
            total_shared_byte_capacity: 63,
            total_private_byte_capacity: 0,
            route: VulkanSharedResidentBufferRoute::SharedHost,
        };
        assert!(
            distributed_calibration_transient_backing_bytes(&devices, &plan)
                .unwrap_err()
                .to_string()
                .contains("shared transient plan declares 63 bytes")
        );

        plan.total_shared_byte_capacity = 64;
        plan.total_private_byte_capacity = 1;
        assert!(
            distributed_calibration_transient_backing_bytes(&devices, &plan)
                .unwrap_err()
                .to_string()
                .contains("private transient plan declares 1 bytes")
        );
    }
