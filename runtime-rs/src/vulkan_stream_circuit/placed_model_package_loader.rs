...
impl VulkanResidentInProcessPlacedModelPackage {
        ...
        F: Fn(&str) -> Result<&'a VulkanComputeDevice, VulkanResidentInProcessPlacedRuntimeError>,
    {
        ...
        let mut shared_host_cache = retained_stores
            ...
            .flatten();
        for slice_indices in physical_slice_groups {
            ...
            let working_set_bytes = logical_device_ids.iter().try_fold(
                0usize,
                |total, device_id| {
                    let plan = residency_plan
                        .device_plans
                        .iter()
                        ...
                        .ok_or_else(|| {
                            VulkanResidentInProcessPlacedRuntimeError::Package(
                                VulkanResidentTokenModelPackageError::new(format!(
                                    "{resource_residency_policy:?} residency plan omitted logical device {device_id:?}"
                                )),
                            )
                        })?;
                    plan.working_set
            ...
            let store_id = format!(
                "{package_id}:physical_store:{physical_device_id}:{execution_scope}"
            );
        ...
        F: Fn(&str) -> Result<&'a VulkanComputeDevice, VulkanResidentInProcessPlacedRuntimeError>,
    {
        ...
        let local_dispatch_count = self.device_slices.iter().try_fold(
            0usize,
            |total, slice| {
                let distributed = distributed_dispatch_indices
                ...
                let model_dispatch_count = slice
                    ...
                    .iter()
                    .filter(|dispatch| {
                        distributed.is_none_or(|indices| {
                            !indices.contains(&dispatch.dispatch_index)
                        })
                    })
                    .count();
                ...
                total
                    ...
                    .ok_or_else(|| {
                        VulkanResidentInProcessPlacedRuntimeError::BackendLoop(VulkanError(
                            "resident feedback planned local dispatch count overflowed"
                                .to_string(),
                        ))
                    })
            },
