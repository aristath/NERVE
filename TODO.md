...
1. Convert all transpiler failures into structured compiler failures. The CLI
   must not leak a traceback, staging output must be removed, and JSONL clients
   must receive one terminal `Failed` event with a useful diagnostic.

2. Discover and represent the DeepSeek V4 graph from topology rather than its
   family name: root `embed`/`norm`/`head`, hyper-connections, compressed sparse
   attention (ratios 4 and 128), indexer tensors, hash-routed early MoE layers,
   sqrt-softplus routing, shared experts, and the 43 repeated processing layers.
   Each structure needs focused negative tests and behavioral contract checks.

3. Preserve the checkpoint's native mixed representations. Routed experts are
...
4. Compile the supplied `encoding/encoding_dsv4.py` behavior into a portable,
...
5. Discover and compile the attached DSpark speculative module, including its
...
6. Complete expert-granular demand-retained residency. Route before admission;
...
7. Add runtime per-layer/per-component representation selection and optimized
...
8. Run real multi-turn DeepSeek conversations with the official thinking and
...
9. Optimize measured inference until steady-state decode reaches at least
...
10. Before every runtime-performance commit, run the established Qwen3.6-35B
...
11. Perform a final adversarial review against `CONCEPT.md`: compiled artifacts
